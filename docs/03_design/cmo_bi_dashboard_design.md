...
Đúng như định hướng, để hệ thống gọn nhẹ và tránh phát sinh xung đột cổng (Port conflict) trong môi trường Docker/WSL, chúng ta **không tạo server riêng** mà sẽ nhúng trực tiếp lớp Dashboard vào tiến trình FastAPI hiện tại của Chainlit thông qua việc kế thừa ASGI Application:

```text
+-------------------------------------------------------------+
|                     CHAINLIT ASGI SERVER                    |
|  (FastAPI instance running under localhost:8000)            |
+-------------------------------------------------------------+
       |                                              |
       v [Mount Static Files]                         v [API Endpoints]
+------------------------------+             +------------------------------+
|      GET /dashboard          |             |   GET /api/dashboard/metrics |
| (Phục vụ file HTML/CSS/JS)   |             |   (Truy vấn số liệu & RAG)   |
+------------------------------+             +------------------------------+
...
### Endpoint 1: Phục vụ Trang Giao diện
*   **Route:** `GET /dashboard`
*   **Mô tả:** Trả về file tĩnh `data/templates/dashboard.html` sử dụng FastAPI `FileResponse`.
...
1.  **Bước 1:** Khởi tạo thư mục mẫu `data/templates/` nếu chưa tồn tại và viết tệp [dashboard.html](file:///wsl.localhost/server/root/marketing-agent-os/data/templates/dashboard.html) theo các token CSS trên.
2.  **Bước 2:** Đăng ký các FastAPI endpoints trong `app.py`, kết nối trực tiếp CSDL qua `SessionLocal` để truy vấn số liệu thực tế của `platform_variants` và `rag_knowledgebase`.
