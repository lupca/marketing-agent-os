...
Việc parse các file Excel/PDF lớn bằng MarkItDown cực kỳ tốn CPU/RAM. Bắt buộc: Hàm `convert_local()` **chỉ được phép thực thi bên trong Celery Worker** (background task), tuyệt đối không được gọi trực tiếp trên luồng chính của FastAPI/Chainlit (`app.py`) để tránh treo UI của người dùng.
