...
- Dễ gây timeout (504 Gateway Timeout) cho ứng dụng Next.js.
...
1. **LangGraph (Publisher Node):** Lưu dữ liệu vào PostgreSQL (`PlatformVariant` với `publish_status = 'scheduled'`) $\rightarrow$ Gọi `celery_app.send_task()` truyền vào ID của bản ghi $\rightarrow$ Trả kết quả ngay cho Sếp trên Dashboard.
